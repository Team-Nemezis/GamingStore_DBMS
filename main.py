...
cur.execute("create table if not exists Main(UID int(4) primary key not null, PASSWORD blob, NAME varchar(40) , TIME_OF_CREATION varchar(60))")


def LoginUser(uid, passw, name):
    cur.execute("select * from Main where UID = {}".format(uid))
    if cur.fetchall() == []:
        date = datetime.datetime.now()
        cur.execute("insert into main values({},AES_ENCRYPT('{}','admin'),'{}', '{}')".format(uid, passw, name, date))
...
def GameStart(uid, Name, time):
    ...
    try:
        ...
        for i in range(10):
            cur.execute("insert into console_check(C_ID,UID) value({},{})".format(i + 1, uid))
        con.commit()
    except:
        pass
    con.commit()
...
def Logout(uid, passw):
    ...
    if passw == t:
        ...
        ye = 'n'
        verify = False
    if ye.lower() == 'y' and verify and payment == 0:
        cur.execute("update table console_check set status = 1 where UID = {}".format(uid))
        cur.execute("update table user_check set status = 0 where UID = {}".format(uid))
        cur.execute("update table log_check set LOGGED_IN = 0 where UID = {}".format(uid))
        cur.execute("update stock_atm set availability = availability + 1 where name = {}".format(uid))
...
while choice != 9876:
    ...
    if choice == 1:
        ...
        if prompt.lower() == 'y':
            ...
            cond = LoginUser(uid, passw, 'default')
            con.commit()
            if cond:
